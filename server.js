...
async function searxng(query) {
  // Query the local SearXNG JSON API and return condensed "title — content (url)" lines.
  ...
  try {
    ...
    const rows = (j.results || []).slice(0, 8).map((x) => {
...
const FIELDS = ["handle","founder_name","num_founders","is_technical","prior_startups","prior_exit",
  "background","full_time","team_size","bootstrapped_vs_funded","failure_reason","source","notes","status"];
...
function finalizeRow(handle, o, source) {
...
async function processHandle(browser, rec) {
  ...
  // 1) SEARCH FIRST via SearXNG — this is where founder info usually lives.
  if (CFG.useSearch) {
    const q = (name ? '"' + name + '" ' : "") + "founder " + (handle ? "@" + handle : "");
    const hits = await searxng(q.trim());
    if (hits && hits.length > 40) { parts.push("WEB SEARCH (founders) for " + (name || handle) + ":\n" + hits); source.push("search"); }
    // a second, targeted query for the harder fields
    const q2 = (name || handle) + " funding raised OR acquired OR shut down";
    const hits2 = await searxng(q2);
    if (hits2 && hits2.length > 40) { parts.push("WEB SEARCH (funding/outcome):\n" + hits2); }
    await sleep(CFG.searchDelayMs || 1200); // pace SearXNG so upstream engines don't throttle
  }
  if (stopRequested) return blankRow(handle, "stopped", "stopped");

  // 2) Try the X profile as corroboration (often login-walled; best-effort).
  try {
  ...
  if (blob.replace(/\s/g, "").length < 120) {
    return blankRow(handle, "empty", "no usable info from search or profile");
  }
  try {
    const raw = await llmChat([{ role: "system", content: SYS }, { role: "user", content: blob.slice(0, 12000) }], true);
    const o = parseJson(raw);
    if (!o) return blankRow(handle, "unknown", "could not parse model output");
    return finalizeRow(handle, o, source.join("+") || "none");
  } catch (e) {
    const m = String(e && e.message ? e.message : e);
    if (stopRequested || /stopped|abort/i.test(m)) return blankRow(handle, "stopped", "stopped");
    return blankRow(handle, "error", m.slice(0, 90));
  }
}
...
<div class="grid">
 <div class="panel"><h2>Configuration</h2>
  ...
  <div class="row2"><div><label>Reasoning effort</label><select id="reasoningEffort"><option value="">provider default</option><option value="low">low</option><option value="medium">medium</option><option value="high">high</option><option value="none">none</option></select></div><div><label>&nbsp;</label><div class="hint" style="margin-top:9px">Fast = no thinking.</div></div></div>
  <div class="checks">
...
<th>Handle</th><th>Founder</th><th>#</th><th>Tech</th><th>Prior</th><th>Exit</th><th>Background</th><th>FT</th><th>Team</th><th>Funding</th><th>Fail reason</th><th>Src</th><th>Notes</th>
</tr></thead><tbody id="tbody"><tr><td colspan="13" class="empty">No results yet. Add your key, paste handles, hit Start.</td></tr></tbody></table></div>
</div>
<script>
...
async function start(){
 ...
 const body={apiKey:$("apiKey").value.trim(),model:$("model").value.trim(),endpoint:$("endpoint").value.trim(),
  ...
  fast:$("fast").checked,useSearch:$("useSearch").checked,resume:$("resume").checked,handles:$("handles").value.split(/\\r?\\n/)};
